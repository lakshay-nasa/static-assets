...
sidebar_position: 69
description: >-
  Power BI is a Microsoft business analytics service that provides interactive
  visualizations and self-service business intelligence dashboards.
title: PowerBI
...
| <div className="path-line"><span className="path-prefix">server_to_platform_instance.`key`.</span><span className="path-main">default_schema</span>&nbsp;<abbr title="Required if server_to_platform_instance is set">❓</abbr></div> <div className="type-name-line"><span className="type-name">string</span></div> | Default Oracle schema applied to unqualified table references found inside ``Oracle.Database(…, Query="…")`` inline native SQL. Set to whatever schema your Oracle ingestion uses as the URN prefix. Only configure this on Oracle servers that need it; other platforms should use plain PlatformDetail. Empty/whitespace values are rejected.  |
...
```javascript
{
  "$defs": {
    ...
    "OraclePlatformDetail": {
      "additionalProperties": false,
      "description": "Oracle-specific platform detail. Adds ``default_schema`` for inline native SQL.",
      "properties": {
        "platform_instance": {
          "anyOf": [
            {
        ...
        "default_schema": {
          "description": "Default Oracle schema applied to unqualified table references found inside ``Oracle.Database(\u2026, Query=\"\u2026\")`` inline native SQL. Set to whatever schema your Oracle ingestion uses as the URN prefix. Only configure this on Oracle servers that need it; other platforms should use plain PlatformDetail. Empty/whitespace values are rejected.",
